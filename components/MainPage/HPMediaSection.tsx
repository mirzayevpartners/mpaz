...
import { IGallery, ILanguage, IVideo } from '@/types';
...
export default async function HPMediaSection({ locale }: { locale: Locale }) {
  ...
  try {
    ...
    sample_gallery = await Gallery.find({ active: true })
      .sort({ customDate: -1 })
      .limit(3)
      .select({ images: { $slice: 1 }, customDate: 1, title: 1 });
  } catch (e) {
    console.log(e);
        ...
        <div className={'flex 1400:!flex-col gap-y-6 gap-x-6 max-w-full'}>
          <div className={'flex flex-1'}>
            <div className={'flex flex-col gap-y-8 1750:!w-[50%]'}>
              ...
              <p className={'text-base leading-[29.82px] text-newsText max-w-[60ch]'}>{t('description')}</p>
